async function handler(m, { conn, args, usedPrefix, command }) {  
  ...
  if (!(who in global.db.data.users)) {  
    return conn.sendMessage(m.chat, {  
      text: `${emoji2} El usuario *${who}* no est¨¢ en la base de datos.`,  
      mentions: [m.sender]  
    }, { quoted: m })  
  }  

  // Inicializar campos para evitar errores
  if (typeof user.dinero !== 'number') user.dinero = 0
  if (typeof user.banco !== 'number') user.banco = 0
  if (typeof global.db.data.users[who].dinero !== 'number') global.db.data.users[who].dinero = 0
  if (typeof global.db.data.users[who].banco !== 'number') global.db.data.users[who].banco = 0
  ...
  const mencion = '@' + who.split('@')[0]  
  
  const mensaje = `  
âœ? Transferiste *${moneda}${count.toLocaleString()} Yenes* a *${nombre}* ${mencion}  
