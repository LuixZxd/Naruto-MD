...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  if (typeof users[senderId].dinero !== 'number') users[senderId].dinero = 0
  ...
  let senderDinero = users[senderId].dinero || 0

  // Inicializar a todos para evitar errores
  Object.keys(users).forEach(id => {
    if (typeof users[id].dinero !== 'number') users[id].dinero = 0
  })
  ...
  let randomUserDinero = users[randomUserId].dinero || 0
  ...
  switch (randomOption) {
    case 0:
      users[senderId].dinero += amountTaken
      users[randomUserId].dinero -= amountTaken
      ...
      break
    case 1:
      let amountSubtracted = Math.min(Math.floor(Math.random() * (senderDinero - minAmount + 1)) + minAmount, maxAmount)
      users[senderId].dinero -= amountSubtracted
      await conn.sendMessage(m.chat, {
      ...
      break
    case 2:
      let smallAmountTaken = Math.min(Math.floor(Math.random() * (randomUserDinero / 2 - minAmount + 1)) + minAmount, maxAmount)
      users[senderId].dinero += smallAmountTaken
      users[randomUserId].dinero -= smallAmountTaken
      await conn.sendMessage(m.chat, {
