...
// Función para calcular nivel según experiencia
function calcularNivel(exp) {
  // Ejemplo: nivel = raíz cuadrada de exp dividido entre 10
  // Puedes ajustar la fórmula a la que prefieras
...
let handler = async function (m, { conn, text, usedPrefix, command }) {
  ...
  // Inicializar campos
  if (user.dinero === undefined) user.dinero = 0
...
➤ Ejemplo: ${usedPrefix + command} ${name2}.18`)
  ...
  user.dinero += 46 // ✅ Ahora usamos dinero
  user.exp += 310
  user.joincount += 25

  // Calcular nivel actual
  const nivel = calcularNivel(user.exp)
