let handler = async (m, { conn, args }) => {
  // Obtener usuarios filtrando:
  // - solo usuarios registrados (registered === true)
  // - y con saldo no negativo (dinero y banco >= 0)
  let users = Object.entries(global.db.data.users)
    .filter(([jid, user]) => user.registered === true && (user.dinero || 0) >= 0 && (user.banco || 0) >= 0)
    .map(([jid, value]) => ({
      jid,
